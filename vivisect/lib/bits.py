...
# FIXME GOES AWAY BECAUSE OF ABOVE CHANGE?
...
def h2b(h):
    ...
    Example:
        x = "41414141"
        doBytesStuff( h2b(x) ) # this got b"AAAA"
    """
...
# FIXME PORT TO BYTES ( ALSO RETURN OFFSET, BITOFF, VALUE )
...
def masktest(s):
    ...
    '''
    maskin = int( s.replace('0','1').replace('x','0'), 2)
    matchval = int( s.replace('x','0'), 2)
    def domask(testval):
